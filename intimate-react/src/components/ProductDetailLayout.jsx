        ...
        <div className="bg-white p-8 rounded-xl shadow-md">
          ...
          {price && (
            <div className="mt-6 bg-green-50 border border-green-200 rounded-xl py-4 px-5">
              ...
              {priceNote && <p className="text-xs text-gray-500 mt-0.5">{priceNote}</p>}
            </div>
          )}
          ...
          <div className="mt-5 space-y-3">
            <div className="flex items-center gap-3">
              <label className="text-sm font-semibold text-gray-700">Quantity (packs):</label>
              <div className="flex items-center border border-gray-300 rounded-xl overflow-hidden">
                ...
                <span className="px-4 py-1 font-semibold text-gray-800 min-w-[3rem] text-center">{qty}</span>
                <button
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {certs.map(({ icon, label }) => (
              <span key={label} className="flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-700 text-xs font-semibold px-3 py-1 rounded-full">
                {icon} {label}
              </span>
            ))}
          </div>
        ...
        <section className="max-w-6xl mx-auto px-5 pb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {reviews.map(({ author, location, rating, comment, avatar }) => (
              <div key={author} className="bg-white rounded-xl shadow-md p-6 flex flex-col">
                <div className="flex gap-0.5 text-yellow-400 text-sm mb-3">
                  {[...Array(rating)].map((_, i) => <span key={i}>★</span>)}
                </div>
                <p className="text-gray-600 text-sm italic flex-1 mb-4">"{comment}"</p>
