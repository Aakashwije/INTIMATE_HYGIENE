import { Building2, Check, GraduationCap, HeartPulse, Landmark, Plane, Smartphone } from "lucide-react";
...
const tiers = [
  {
    ...
    features: ["Standard packaging", "Custom logo printing available", "Delivery within 5 business days", "Email support"],
  },
  {
    name: "Business",
    ...
    highlight: true,
    features: ["Custom branded packaging", "Free delivery island-wide", "1 free wall-mount dispenser", "Priority WhatsApp support"],
  },
  {
    name: "Enterprise",
    ...
    features: ["Custom branded packaging", "Free delivery island-wide", "5 free wall-mount dispensers", "Dedicated account manager"],
  },
  {
    name: "Premium",
    volume: "5,000+ packs",
    ...
    features: ["Fully custom packaging & branding", "Priority manufacturing slots", "Unlimited dispensers", "SLA-backed delivery guarantee"],
  },
];
...
const sectors = [
  ...
  { icon: Plane,          label: "Airlines & Airports" },
...
const steps = [
  { num: "01", title: "Submit Enquiry",      desc: "Fill the form below. Your details are sent directly to us via WhatsApp for instant response." },
  { num: "02", title: "Receive Custom Quote", desc: "We'll get back to you within 2 hours with pricing, sample options, and delivery timelines." },
  { num: "03", title: "Confirm & Deliver",   desc: "Approve your order and we handle everything — packaging, branding, and timely delivery." },
];

const initForm = { company: "", name: "", email: "", phone: "", product: "", volume: "", message: "" };
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-5 drop-shadow-md">{t.b2bHeroTitle}</h1>
          <p className="text-lg md:text-xl leading-relaxed opacity-95">{t.b2bHeroSub}</p>
        </div>
      </section>
      ...
      <section className="py-16 px-5 max-w-5xl mx-auto text-center">
        ...
        <p className="text-gray-500 mb-10">Trusted by businesses across Sri Lanka</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {sectors.map(({ icon: Icon, label }) => (
            <div
              key={label}
              className="flex flex-col items-center gap-3 bg-white rounded-xl p-5 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300"
            >
      ...
      <section className="py-16 px-5 bg-gray-50">
        <div className="max-w-6xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-gray-800 mb-3">Volume Pricing</h2>
          <p className="text-gray-500 mb-10">The more you order, the more you save. All prices are per pack.</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {tiers.map(({ name, volume, price, unit, color, badge, highlight, features }) => (
              ...
              >
                ...
                <span className={`text-xs font-semibold px-3 py-1 rounded-full self-start mb-3 ${badge}`}>{name}</span>
                ...
                <ul className="mt-4 space-y-2 flex-1">
                  {features.map((f) => (
                    <li key={f} className="text-sm text-gray-600 flex items-start gap-2">
                      <Check className="w-4 h-4 text-[#28a745] shrink-0 mt-0.5" />
                      {f}
                    </li>
                  ))}
                </ul>
      ...
      <section className="py-10 px-5 bg-gray-50">
        <div className="max-w-md mx-auto text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <h3 className="text-xl font-bold text-gray-800">Quick Reorder Anytime</h3>
          </div>
          <p className="text-gray-500 text-sm mb-5">
            Print this QR code on your invoices or packaging inserts so clients can reorder instantly.
          </p>
          <img
          ...
          />
          <p className="text-xs text-gray-400 mt-3">Links to our website for fast reordering</p>
        </div>
      </section>
      ...
      <section id="b2b-form" className="py-16 px-5 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-md p-5 sm:p-8 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <input type="email" name="email" placeholder="Email Address" value={form.email} onChange={onChange} required className="border border-gray-300 rounded-lg px-4 py-3 text-gray-800 focus:outline-none focus:border-[#28a745] transition-colors" />
            <input type="tel" name="phone" placeholder="Phone / WhatsApp Number" value={form.phone} onChange={onChange} required className="border border-gray-300 rounded-lg px-4 py-3 text-gray-800 focus:outline-none focus:border-[#28a745] transition-colors" />
          </div>
          <select name="product" value={form.product} onChange={onChange} required className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-700 focus:outline-none focus:border-[#28a745] transition-colors">
          ...
          <select name="volume" value={form.volume} onChange={onChange} required className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-700 focus:outline-none focus:border-[#28a745] transition-colors">
          ...
          <textarea name="message" rows={4} placeholder="Additional requirements (branding, delivery schedule, etc.)" value={form.message} onChange={onChange} className="w-full border border-gray-300 rounded-lg px-4 py-3 text-gray-800 focus:outline-none focus:border-[#28a745] transition-colors resize-none" />
          <button type="submit" className="w-full flex items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-3 rounded-xl hover:bg-[#1ea952] transition-colors text-lg">
