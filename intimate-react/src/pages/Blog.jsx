import { Baby, Briefcase, Bug, Building2, HeartPulse, Leaf, Microscope, Newspaper, Plane, Recycle, ShoppingBag, Sparkles, Star } from "lucide-react";
...
const articles = [
  {
    id: 1, category: "Health & Hygiene", readTime: "4 min read", date: "April 20, 2025",
    icon: Bug, featured: true,
    title: "Why You Should Never Sit on a Public Toilet Seat",
    excerpt: "Studies reveal that a single public toilet seat can harbor up to 295 different types of bacteria per square inch. Here's what you need to know — and what to do about it.",
    content: [
  ...
  {
    id: 2, category: "Travel Tips", readTime: "3 min read", date: "March 15, 2025",
    icon: Plane, featured: false,
    title: "5 Travel Hygiene Habits That Could Save Your Life",
    excerpt: "Seasoned travelers know hygiene on the road requires extra vigilance. These 5 habits make the difference between staying healthy and falling ill.",
    content: [
  ...
  {
    id: 3, category: "Industry", readTime: "5 min read", date: "February 8, 2025",
    icon: Building2, featured: false,
    title: "How Hotels Are Embracing Eco-Friendly Hygiene Solutions",
    excerpt: "The hospitality industry is pivoting toward sustainable hygiene. Here's how top hotels in Sri Lanka and worldwide are making the switch.",
    content: [
  ...
  {
    id: 4, category: "Science", readTime: "4 min read", date: "January 25, 2025",
    icon: Microscope, featured: false,
    title: "The Truth About Bacteria on Public Toilet Seats",
    excerpt: "The science is more alarming than most people know. We break down exactly what lives on public toilet surfaces.",
    content: [
  ...
  {
    id: 5, category: "Healthcare", readTime: "3 min read", date: "December 12, 2024",
    icon: HeartPulse, featured: false,
    title: "Why Hospitals Are Switching to Disposable Seat Covers",
    excerpt: "Infection control is a matter of life and death in healthcare. Disposable toilet seat covers have become standard protocol in top hospitals worldwide.",
    content: [
  ...
  {
    id: 6, category: "Lifestyle", readTime: "3 min read", date: "November 5, 2024",
    icon: Baby, featured: false,
    title: "Protecting Your Children in Public Restrooms",
    excerpt: "Children's immune systems are still developing, making them far more vulnerable. Here's how to keep them safe.",
    content: [
  ...
  {
    id: 7, category: "Eco Living", readTime: "4 min read", date: "October 18, 2024",
    icon: Recycle, featured: false,
    title: "Are Disposable Hygiene Products Bad for the Environment?",
    excerpt: "A common concern — but the answer might surprise you. Here's the science behind eco-friendly, water-soluble toilet seat covers.",
    content: [
  ...
  {
    id: 8, category: "Health & Hygiene", readTime: "5 min read", date: "September 3, 2024",
    icon: Briefcase, featured: false,
    title: "Office Hygiene: The Hidden Risk in Your Workplace Bathroom",
    excerpt: "Your office bathroom is shared by dozens of colleagues daily. The hygiene risks are real — and most companies aren't doing enough.",
    content: [
...
const categoryColors = {
  "Health & Hygiene": { bg: "bg-green-100",  text: "text-green-700",  border: "border-green-200",  accent: "bg-green-500"  },
  "Travel Tips":      { bg: "bg-blue-100",   text: "text-blue-700",   border: "border-blue-200",   accent: "bg-blue-500"   },
  Industry:           { bg: "bg-purple-100", text: "text-purple-700", border: "border-purple-200", accent: "bg-purple-500" },
  Science:            { bg: "bg-orange-100", text: "text-orange-700", border: "border-orange-200", accent: "bg-orange-500" },
  Healthcare:         { bg: "bg-red-100",    text: "text-red-700",    border: "border-red-200",    accent: "bg-red-500"    },
  Lifestyle:          { bg: "bg-pink-100",   text: "text-pink-700",   border: "border-pink-200",   accent: "bg-pink-500"   },
  "Eco Living":       { bg: "bg-teal-100",   text: "text-teal-700",   border: "border-teal-200",   accent: "bg-teal-500"   },
};

const allCategories = ["All", ...Object.keys(categoryColors)];

const quickFacts = [
  { icon: Bug,      stat: "295+", label: "Bacteria types per sq inch on toilet seats" },
  { icon: Sparkles, stat: "99%",  label: "Pathogen contact reduction with a cover" },
  { icon: Leaf,     stat: "100%", label: "Biodegradable & water-soluble" },
  { icon: Building2,stat: "200+", label: "Businesses trust Hygenc island-wide" },
];
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4 drop-shadow-md">{t.blogHeroTitle}</h1>
          <p className="text-lg md:text-xl leading-relaxed opacity-90 mb-8">{t.blogHeroSub}</p>
          <div className="flex flex-wrap justify-center gap-6">
            {[["8","Expert Articles"],["10K+","Monthly Readers"],["7","Categories"],["2025","Latest Issue"]].map(([v,l]) => (
                ...
                <div className="relative z-10 p-8 md:p-12 flex flex-col md:flex-row gap-8 items-center">
                  ...
                  <div className="text-white text-left flex-1">
                    <div className="flex flex-wrap items-center gap-3 mb-4">
                      ...
                      <span className="text-white/60 text-xs">{featured.date} · {featured.readTime}</span>
                    </div>
                    <h2 className="text-2xl md:text-3xl font-bold mb-3 leading-snug">{featured.title}</h2>
                    <p className="text-white/80 leading-relaxed mb-5 text-sm md:text-base">{featured.excerpt}</p>
                    {expanded === featured.id && (
                      <div className="space-y-3 border-t border-white/20 pt-4 mb-5">
                        {featured.content.map((para, i) => (
                          <p key={i} className="text-white/75 leading-relaxed text-sm">{para}</p>
                        ))}
                      </div>
                    )}
                    <div className="flex flex-wrap gap-3">
                      ...
                      >
                        {expanded === featured.id ? `← ${t.showLess}` : `${t.readMore} →`}
                      </button>
                      <Link to="/products" className="flex items-center gap-2 px-5 py-2.5 border-2 border-white/40 text-white font-semibold rounded-xl text-sm hover:bg-white/10 transition-colors">
            ...
            {allCategories.map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-semibold transition-all duration-200 ${
                  activeCategory === cat ? "bg-[#28a745] text-white shadow-md scale-105" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}
              >
                {cat}
              </button>
          ...
          {filtered.map((article, i) => {
            ...
            return (
              <Reveal key={article.id} delay={i * 80}>
                <article className={`bg-white rounded-2xl shadow-md overflow-hidden border ${colors.border} hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col h-full`}>
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 text-xs mt-1">{article.date} · {article.readTime}</p>
                      </div>
                    </div>
                    <h2 className="text-base font-bold text-gray-800 mb-2 leading-snug">{article.title}</h2>
                    ...
                    <div className={`overflow-hidden transition-all duration-500 ${isOpen ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}>
                      <div className="space-y-3 border-t border-gray-100 pt-4 pb-2">
                        {article.content.map((para, j) => (
                          <p key={j} className="text-gray-600 leading-relaxed text-xs">{para}</p>
                        ))}
                      </div>
                    </div>
                    <button
        ...
        <Reveal variant="zoom">
          <div className="mt-14 bg-gradient-to-br from-[#28a745] to-[#1d7a34] rounded-2xl p-8 text-white text-center shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-xl md:text-2xl font-bold mb-2">Ready to protect yourself?</h3>
              <p className="text-white/80 mb-6 text-sm max-w-md mx-auto">
                Join 10,000+ Sri Lankans who choose Hygenc. Starting from LKR 150.
              </p>
              <div className="flex flex-wrap justify-center gap-3">
                <Link to="/products" className="flex items-center gap-2 px-6 py-3 bg-white text-[#28a745] font-bold rounded-xl text-sm hover:bg-green-50 transition-colors shadow-md">
        ...
        <Reveal>
          <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
            {quickFacts.map(({ icon: Icon, stat, label }) => (
              <div key={stat} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 text-center hover:shadow-md transition-shadow">
        ...
        <Reveal>
          <div className="mt-8 flex flex-col md:flex-row items-center gap-5 bg-gray-50 border border-gray-200 rounded-2xl p-6">
            ...
            <div className="flex-1 text-center md:text-left">
              <h4 className="font-bold text-gray-800 mb-1">Running a hotel, clinic, or office?</h4>
              <p className="text-gray-500 text-sm">Get bulk pricing, free dispensers, and custom branding with our Enterprise Pack.</p>
            </div>
            <Link to="/b2b" className="shrink-0 px-5 py-2.5 bg-gray-900 text-white font-semibold rounded-xl text-sm hover:bg-gray-700 transition-colors whitespace-nowrap">
              Enquire B2B →
            </Link>
          </div>
        </Reveal>
