  ...
  return (
    <>
      ...
      <main className="max-w-6xl mx-auto py-16 px-5 text-center">
        <h1 className="text-3xl font-bold text-gray-900 mb-3">{t.ourProducts}</h1>
        <p className="text-gray-500 text-lg mb-6">{t.ourProductsSub}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {productDefs.map(({ img, titleKey, descKey, badge, urgencyKey, price, priceNote, whatsappMsg, link }) => (
            <div key={titleKey} className="relative bg-white rounded-2xl shadow-lg p-7 text-center overflow-hidden hover:-translate-y-1 hover:shadow-2xl transition-all duration-300 group">
              {/* Hover radial bg */}
              ...
              {/* Image */}
              <img src={img} alt={t[titleKey]} className="w-full max-h-56 object-cover rounded-2xl mb-4 group-hover:scale-105 transition-transform duration-300 relative z-10" />

              <h2 className="text-2xl font-semibold text-[#33691E] mb-3 group-hover:text-[#7CB342] transition-colors relative z-10">{t[titleKey]}</h2>
              <p className="text-gray-500 mb-4 relative z-10">{t[descKey]}</p>
              ...
              <div className="relative z-10 flex flex-col gap-2">
                ...
                <Link to={link} className="w-full py-2.5 border-2 border-[#7CB342] text-[#7CB342] font-semibold rounded-xl hover:bg-[#7CB342] hover:text-white transition-all duration-300 text-sm">
                  {t.viewDetails}
                </Link>
              </div>
            </div>
        ...
        <div className="mt-12 bg-gray-900 text-white rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-xl font-bold mb-1">{t.b2bBanner} 🏢</h3>
            <p className="text-gray-400 text-sm">{t.b2bBannerSub}</p>
          </div>
          <Link to="/b2b" className="flex-shrink-0 px-6 py-3 bg-[#28a745] text-white font-bold rounded-xl hover:bg-[#218838] transition-colors whitespace-nowrap">
            {t.enquireB2B} →
          </Link>
        </div>
      </main>
