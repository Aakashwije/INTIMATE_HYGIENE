...
const productDefs = [
  {
    ...
    whatsappMsg: "Hello! I want to order the Single Use Pack. Please share pricing and availability.",
    link: "/products/1",
  },
  {
    id: "travel",
    ...
    whatsappMsg: "Hello! I want to order the Travel Pack (Waterproof). Please share pricing and availability.",
    link: "/products/2",
  },
  {
    id: "enterprise",
    ...
    whatsappMsg: "Hello! I want to enquire about the Enterprise Pack bulk pricing. Please share details.",
    link: "/products/3",
  },
];
  ...
  return (
    <>
      ...
      <section
        ...
        style={{ background: 'linear-gradient(135deg, rgba(13,63,26,0.9), rgba(40,167,69,0.85)), url("/hero.jpg") no-repeat center/cover' }}
      >
        ...
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 mb-4 rounded-full bg-white/15 backdrop-blur-sm border border-white/30 text-xs font-semibold">
            <span className="text-yellow-300">★★★★★</span> 4.9/5 · 124+ reviews
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-5 drop-shadow-md">{t.ourProducts}</h1>
          <p className="text-lg md:text-xl leading-relaxed opacity-95">{t.ourProductsSub}</p>
        </div>
      </section>
      ...
      <main className="max-w-6xl mx-auto py-16 px-5 text-center">
        <Reveal>
          <h2 className="text-3xl font-bold text-gray-900 mb-3">Pick your perfect pack</h2>
          <p className="text-gray-500 text-lg mb-6">Loved by 10,000+ Sri Lankans · Free delivery over LKR 3,000</p>
        </Reveal>
          ...
          {productDefs.map((p, i) => (
            <Reveal key={p.titleKey} delay={i * 120}>
              <div className="relative bg-white rounded-2xl shadow-lg overflow-hidden hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 group h-full flex flex-col border border-gray-100">
                {/* Image section — edge-to-edge so badges sit cleanly on top */}
                <div className="relative overflow-hidden">
                  <img src={p.img} alt={t[p.titleKey]} loading="lazy" className="w-full h-56 object-cover group-hover:scale-110 transition-transform duration-500" />

                  {/* Hover radial overlay on image */}
                  ...
                  {/* Product-type badge — top right, inside image */}
                  ...
                  {/* Urgency badge — top left, inside image */}
                  {p.urgencyKey && (
                ...
                <div className="flex flex-col flex-1 p-6 text-center">
                  <h2 className="text-2xl font-bold text-gray-800 mb-2 group-hover:text-[#28a745] transition-colors">{t[p.titleKey]}</h2>
                  <p className="text-gray-500 mb-4 flex-1 text-sm leading-relaxed">{t[p.descKey]}</p>
                  ...
                  <div className="mb-5 bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl py-3 px-4">
                    <p className="text-3xl font-bold text-[#28a745]">{p.priceLabel}</p>
                    <p className="text-xs text-gray-500 mt-0.5">{p.priceNote}</p>
                  </div>
                  ...
                  <Link to={p.link} className="w-full py-2.5 border-2 border-[#7CB342] text-[#7CB342] font-semibold rounded-xl hover:bg-[#7CB342] hover:text-white transition-all duration-300 text-sm">
                    {t.viewDetails}
                  </Link>
                </div>
                </div>
        ...
        <Reveal>
          <div className="mt-8 flex items-center justify-center gap-3 text-sm text-gray-500">
            <Link to="/compare" className="text-[#28a745] font-bold hover:underline">
              {t.compareProducts}
            </Link>
          </div>
        </Reveal>
        ...
        <Reveal variant="zoom">
          <div className="mt-12 bg-gradient-to-br from-gray-900 to-gray-800 text-white rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10">
              <h3 className="text-xl md:text-2xl font-bold mb-1">{t.b2bBanner} 🏢</h3>
              <p className="text-gray-400 text-sm">{t.b2bBannerSub}</p>
            </div>
            <Link to="/b2b" className="btn-shimmer relative z-10 flex-shrink-0 px-6 py-3 bg-[#28a745] text-white font-bold rounded-xl hover:bg-[#218838] hover:-translate-y-0.5 transition-all whitespace-nowrap shadow-lg">
              {t.enquireB2B} →
            </Link>
          </div>
        </Reveal>
