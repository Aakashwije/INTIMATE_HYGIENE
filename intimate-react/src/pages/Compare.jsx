          ...
          <table className="w-full border-separate border-spacing-0 text-xs sm:text-sm min-w-[560px]">
                ...
                {productDefs.map((p) => (
                  <th key={p.name} className="pb-6 px-1.5 sm:px-3 align-top">
                    <div className="bg-white rounded-2xl shadow-md p-3 sm:p-4 flex flex-col items-center gap-1.5 sm:gap-2">
                      ...
                      <Link
                        to={p.link}
                        className="w-full text-center py-1 sm:py-1.5 border border-[#7CB342] text-[#7CB342] font-semibold rounded-xl text-[10px] sm:text-xs hover:bg-[#7CB342] hover:text-white transition-all"
                      >
                        {t.viewDetails}
                      </Link>
                    </div>
            ...
            <tbody>
              {rows.map(({ label, values, best }, ri) => (
                ...
                >
                  <td className="py-3 px-2 sm:px-4 font-semibold text-gray-700 rounded-l-xl text-xs sm:text-sm">
                    {label}
                  </td>
                  {values.map((val, ci) => {
                    const isGood = val === "✔";
                    ...
                    return (
                      <td
                        key={ci}
                        className={`py-3 px-2 sm:px-4 text-center font-medium rounded-xl ${isGood ? "text-[#28a745]" : ""} ${isBad ? "text-gray-300" : ""} ${isBest ? "bg-green-50 text-[#28a745] font-bold" : ""}`}
                      >
                        {isGood ? (
