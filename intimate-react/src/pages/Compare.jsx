...
// rows: [label, [val1, val2, val3], highlight index]
const rows = [
  {
    label: "Price",
      ...
      >
        ...
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-lg opacity-90">
            Find the right cover for your needs — side-by-side.
          </p>
        </div>
      </section>
      ...
      <main className="max-w-5xl mx-auto px-4 py-14">
          ...
          <table className="w-full border-separate border-spacing-0 text-sm">
            <thead>
              <tr>
                ...
                {products.map((p) => (
                  <th key={p.name} className="pb-6 px-3 align-top">
                    <div className="bg-white rounded-2xl shadow-md p-4 flex flex-col items-center gap-2">
                      ...
                      <a
                        ...
                        className="mt-1 w-full text-center py-2 bg-[#25D366] text-white font-semibold rounded-xl text-xs hover:bg-[#1ea952] transition-colors"
                      >
                        Order on WhatsApp
                      </a>
                      <Link
                        to={p.link}
                        className="w-full text-center py-1.5 border border-[#7CB342] text-[#7CB342] font-semibold rounded-xl text-xs hover:bg-[#7CB342] hover:text-white transition-all"
                      >
                        View Details
                      </Link>
                    </div>
            ...
            <tbody>
              {rows.map(({ label, values, best }, ri) => (
                ...
                >
                  <td className="py-3 px-4 font-semibold text-gray-700 rounded-l-xl">
                    {label}
                  </td>
                  {values.map((val, ci) => {
                    const isGood = val === "✔";
                    ...
                    return (
                      <td
                        key={ci}
                        className={`py-3 px-4 text-center font-medium rounded-xl
                          ${isGood ? "text-[#28a745]" : ""}
                          ${isBad ? "text-gray-300" : ""}
                          ${isBest ? "bg-green-50 text-[#28a745] font-bold" : ""}
                        `}
                      >
                        {isGood ? (
        ...
        <div className="mt-12 bg-gray-900 text-white rounded-2xl p-8 text-center">
          ...
          <p className="text-gray-400 text-sm mb-5">
            Message us on WhatsApp — we'll recommend the right pack for your use
            case in minutes.
          </p>
          <a
